...
class Detectron2GradCAM():
  ...
  def _set_input_image(self, img_path):
    ...
    #       [self.cfg.INPUT.MIN_SIZE_TEST, self.cfg.INPUT.MIN_SIZE_TEST], self.cfg.INPUT.MAX_SIZE_TEST
    #   )
      transformed_img = self.transform_gen.get_transform(self.image).apply_image(self.image)
      self.input_tensor = torch.as_tensor(transformed_img.astype("float32").transpose(2, 0, 1)).requires_grad_(True)
  
  def get_cam(self, input_image_dict,target_instance, layer_name, grad_cam_instance):
      """
    ...
    #   input_image_dict = {"image": self.input_tensor, "height": self.image_height, "width": self.image_width}
      ...
      self.grad_cam = grad_cam_instance(self.model, layer_name)
    
      with self.grad_cam as cam:
        cam_data, cam_orig, output , results= cam(input_image_dict, target_instance=target_instance)
      
      output_dict = self.get_output_dict(cam_data, output, target_instance, input_image_dict)

      return output_dict, cam_orig, results
    
  def get_output_dict(self, cam, output, target_instance, input_image_dict = None):
      image_dict = {}
      #permute numpy image from CHW to HWC

      image_dict["image"] =  input_image_dict[0]["image"].numpy().transpose(1,2,0) # if self.image is None else self.image
      image_dict["cam"] = cam
