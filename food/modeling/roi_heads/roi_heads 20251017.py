        ...
        if self.withvae:
            ...
            if self.with_text_clip and self.training:
                ...
                if self.with_text_clip:
                    ...
                    # #add image clip feature for class 21
                    # if self.with_image_clip:
                    #     for idx, gt_label in enumerate(gt_classes_new):
                    #         if gt_label == 21:
                    #             x0, y0, x1, y1 = gt_boxes[idx].round().long()
                    #             roi_img = images[gt_image_idx[idx]][:, y0:y1, x0:x1]
                    #             roi_img = (roi_img - roi_img.min()) / (roi_img.max()- roi_img.min()) * 255
                    #             roi_pil = Image.fromarray(roi_img.byte().permute(1, 2, 0).cpu().numpy())
                    #             img_feat = self.clip_model.encode_image(self.clip_preprocess(roi_pil).unsqueeze(0).to('cuda')).to(torch.float32)
                    #             # text_features[idx,] = img_feat.squeeze()
                    #             img_feat = self.linear(img_feat)
                    #             text_selected[idx] = img_feat

                    
                    # x_recon_mean = x_recon_mean + text_selected * 0.2
                    # x_recon_mean = x_recon_mean + text_features_2 * 0.2
                    # if text_features is not None:
                    #     vae_features = torch.cat([vae_features, text_features], dim=1)
                    # 将VAE的输入特征与CLIP提取的文本特征进行融合（这里简单拼接作为示例，你可以尝试其他更复杂的融合方式）
                    # vae_features = torch.cat([vae_features, clip_text_features], dim=1)     
            if self.with_image_clip and self.training:
                ...
                if len(selected_proposal_boxes) > 0:
                    ...
                    if is_replace_feature_with_noise:
                        ...
                        gaosi = False
                        if gaosi:                            
                            # x = torch.randn(1027,77).to(device)
                            x = torch.randint(0, 1024, (1024,77)).to(device)
                            bg_features = self.clip_model.encode_text(x.long().to(device)).float()
                        else:                          
                        ...
                        #x_recon_mean = x_recon_mean + bg_features 
                        ...
                        linearf = False
                        onedim = False
                        bg_features_new = feature_pooled[mask]

                        if onedim:
                            
                            # fg_feature = feature_pooled[~mask]
                            # fg_feature_new = torch.mean(fg_feature, dim=0, keepdim=True)
                            # bg_features_new = torch.mean(bg_features, dim=0, keepdim=True)
                            # diff = fg_feature_new - bg_features_new
                            # # 对差进行平方操作
                            ...
                            mean_tensor = torch.mean(bg_features_new, dim=0, keepdim=True)
                            # 计算每个元素与均值张量的差
                            diff = bg_features_new - mean_tensor
                            # 计算差的平方
                            squared_diff = torch.square(diff)
                            # 计算平方差的和，得到形状为 (1, n) 的张量
                            sum_squared_diff = torch.sum(squared_diff, dim=0, keepdim=True)
                            # 对平方和开平方，得到每个元素与均值的欧氏距离，形状为 (1, n)
                            euclidean_distances = torch.sqrt(sum_squared_diff)
                            # 计算平均的欧氏距离
                            average_distance = torch.mean(euclidean_distances)
                            print(average_distance)

                        if zero:
                            
                            fg_feature = feature_pooled[~mask]
                            padding_tensor_fg = torch.zeros(bg_features_new.shape[0] - padding_size, bg_features_new.shape[1])                      
                            fg_feature_new = torch.cat((fg_feature, padding_tensor_fg.to(device)), dim=0)                                                            
                        ...
                        if linearf:
                            ...
                            fg_feature = feature_pooled[~mask]
                            x1, n = bg_features_new.shape
                            m, n_fg = fg_feature.shape
                            linear_layer = nn.Linear(m, x1).to(device)
                            flattened_fg = fg_feature.view(-1,m).to(device)
                            # 通过 Linear 层进行线性变换
                            mapped_flattened = linear_layer(flattened_fg).to(device)
                            # 将映射后的一维张量重塑为 (x1, n) 的二维张量
                            fg_feature_new = mapped_flattened.view(x1, fg_feature.shape[1])
                            diff = fg_feature_new - bg_features_new
                            # 对差进行平方操作
                            squared_diff = torch.square(diff)
                            # 对平方差求和
                        ...
                        #bg_features = feature_pooled[mask]
            ...
            if self.training:
                ...
                # if zero:
                #     fg_feature = feature_pooled[~mask]
                #     padding_tensor_fg = torch.zeros(bg_features.shape[0] - padding_size, bg_features.shape[1])                      
                #     fg_feature_new = torch.cat((fg_feature, padding_tensor_fg.to(device)), dim=0)                                                            
                    
                #     diff = fg_feature_new - bg_features
                #     # 对差进行平方操作
                #     squared_diff = torch.square(diff)
                #     # 对平方差求和
                #     sum_squared_diff = torch.sum(squared_diff)
