...
class FastRCNNOutputs(object):
    ...
    def losses(self):
        ...
        """
        cross_entropy_loss_func = nn.CrossEntropyLoss(reduction="mean")
        loss_cls = cross_entropy_loss_func(self.pred_class_logits, self.gt_classes)
        return {
            "loss_cls": loss_cls, #self.softmax_cross_entropy_loss(),
...
class CosineFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        return fast_rcnn_inference(
            boxes,
            scores,
            image_shapes, # type: ignore
            self.test_score_thresh,
            self.test_nms_thresh,
            self.test_topk_per_image,
            self.vis_iou_thr, # type: ignore
        )
...
def kmeans_gpu(x, k, niter=20):
    x_cpu = x.detach().cpu().numpy()
    kmeans = KMeans(n_clusters=k,random_state=0).fit(x_cpu)
    labels = kmeans.labels_
    labels_tensor = torch.tensor(labels, device=x.device)
    centorids = torch.tensor(kmeans.cluster_centers_, device=x.device)
    return labels_tensor, centorids

def affinity_propagation_gpu(features, preference=None, device=torch.device('cpu')):
    """
    使用Affinity Propagation算法进行聚类，并返回聚类标签和聚类中心。

    参数:
    features (torch.Tensor): 输入的特征数据，形状为 (样本数量, 特征维度)。
    preference (float, 可选): Affinity Propagation算法的偏好参数，默认为None，表示使用中位数相似性。
    device (torch.device, 可选): 指定数据所在的设备（CPU或GPU），默认为CPU。

    返回:
    labels (torch.Tensor): 聚类标签，形状为 (样本数量, )，每个元素表示对应样本所属的聚类编号。
    centroids (torch.Tensor): 聚类中心，形状为 (聚类数量, 特征维度)，表示每个聚类的中心位置。
    """
    # 将特征数据从torch.Tensor转换为numpy数组，并移动到CPU上（因为sklearn要求输入的是numpy数组且在CPU内存中）
    features_cpu_np = features.cpu().detach().numpy()
    # 使用AffinityPropagation进行聚类
    af = AffinityPropagation(preference=preference).fit(features_cpu_np)
    # 获取聚类标签（numpy数组格式）
    labels_np = af.labels_
    # 获取聚类中心对应的索引（每个聚类对应的一个索引标识）
    centroids_indices = np.unique(labels_np)
    # 计算聚类中心（根据索引获取对应数据并求平均得到聚类中心）
    centroids_np = [features_cpu_np[labels_np == i].mean(axis=0) for i in centroids_indices]
    centroids_np = np.array(centroids_np)
    # 将聚类标签和聚类中心转换为torch.Tensor，并将聚类中心移动到指定设备上
...
# 计算对比损失
def contrastive_loss(features, labels, centroids, margin=1.0):
    """
    计算对比损失

    参数:
    - features: Tensor，特征向量，形状为 (N, D)
    - labels: Tensor，聚类标签，形状为 (N,)
    - margin: float，距离阈值
    ...
    """
    # 计算特征向量之间的欧几里得距离矩阵
    distances = F.pairwise_distance(features.unsqueeze(1), features.unsqueeze(0))
    
    # 生成标签掩码矩阵
    labels_equal = (labels.unsqueeze(0) == labels.unsqueeze(1)).float()

    # 相似样本对的损失
    loss_positive = labels_equal * distances.pow(2)

    # 不相似样本对的损失
    loss_negative = (1 - labels_equal) * torch.clamp(margin - distances, min=0.0).pow(2)

    # 总损失
    # loss = 0.5 * (loss_positive + loss_negative).sum() / features.size(0) /features.size(0)
    loss = 0.01 * (loss_positive + loss_negative).sum() / features.size(0)
...
class FOODFastRCNNOutputLayers(CosineFastRCNNOutputLayers):
    ...
    def __init__(
        ...
        **kargs
        ...
        c_loss = self.c_loss 
        # if c_loss:
        self.contrastive_head = ContrastiveHead(dim_in=self.cls_score.in_features, feat_dim=ic_loss_out_dim)
        self.supcon_loss = SupConLoss(temperature=ic_loss_queue_tau, iou_threshold=ic_loss_batch_iou_thr)             
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            '_ALPHA': cfg._ALPHA,
            'cons_loss': cfg.cons_loss,
            'background_loss': cfg.background_loss,
            'VAE': cfg.VAE,
        })
        return ret

    def forward(self, feats, feat_cons = None):
        # support shared & sepearte head
        if isinstance(feats, tuple):
            reg_x, cls_x = feats
        else:
        ...
        with_vae = self.withvae
        c_loss = self.c_loss

        contrastive_feat = None
        if with_vae and feat_cons is not None:
            # vae_feat = 0
            alpha = self.alpha            
            # if c_loss:
            #     feat_cons = torch.norm(cls_x, p=2, dim=1).unsqueeze(1).expand_as(feat_cons)
            #     feat_cons = feat_cons.div(feat_cons + 1e-5)
            #     contrastive_feat = self.contrastive_head(feat_cons)
            cls_x = alpha * cls_x + (1 - alpha) * feat_cons # type: ignore

        x_normalized = cls_x.div(x_norm + 1e-5)

        
        if c_loss:
            contrastive_feat = self.contrastive_head(x_normalized)
        ...
        if c_loss:
            return scores, proposal_deltas, mlp_feat, contrastive_feat
        else:
            return scores, proposal_deltas, mlp_feat, None
    ...
    def get_up_loss(self, scores, gt_classes, un_id):
        ...
        return {"loss_cls_unknown_{}".format(un_id): self.up_loss_weight * loss_cls_up}
    ...
    def losses(self, predictions, proposals, input_features=None):
        ...
        scores, proposal_deltas, mlp_feat, contrastive_feat = predictions
        ...
        if len(proposals):
            ...
            # gt_labels = 
            # gt_labels = [labels_names_list[x.detach() - 1] for x in gt_classes]
        else:
        ...
        # background loss add by wtx
        background_loss = self.background_loss 
        if background_loss and self.training:

            back_mask = gt_classes != self.num_classes
            back_mlp_feat = mlp_feat[back_mask, ]
            back_ious = ious[back_mask, ]
            # num_clusters = 6
            # if back_mlp_feat.shape[0] > num_clusters:
                # kmeans of back_mlp_feat
                # labels, centorids = kmeans_gpu(back_mlp_feat, num_clusters)
                
            # affinity propagation of back_mlp_feat
            labels, _ = affinity_propagation_gpu(back_mlp_feat)
            
            #contrastive loss
            # loss_contrastive = contrastive_loss(back_mlp_feat, labels, centorids)
            
            # by xzm: contrastive loss                
            contrastive_feature = self.contrastive_head(back_mlp_feat)
            loss_contrastive = self.supcon_loss(contrastive_feature, labels, back_ious)
            
            losses['loss_background'] = loss_contrastive
            # else:
            #     losses['loss_background'] = torch.tensor(0.0).to(back_mlp_feat.device)

        # end of background loss
        # up loss
        ...
        c_loss = self.c_loss
        if c_loss and contrastive_feat is not None:
            features = contrastive_feat
            labels = gt_classes
            ious_for_supcon = ious
            supcon_loss_value = self.supcon_loss(features, labels, ious_for_supcon)
            losses["loss_vae_constrive"] = supcon_loss_value
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        """
        super(FastRCNNOutputLayers, self).__init__() # type: ignore
        ...
        # background class
        # (hence + 1)
        self.cls_score = nn.Linear(input_size, num_classes + 1) # type: ignore
        num_bbox_reg_classes = 1 if cls_agnostic_bbox_reg else num_classes
        self.bbox_pred = nn.Linear(input_size, num_bbox_reg_classes * box_dim) # type: ignore
