        ...
        for fileid in fileids_:
            ...
            for obj in tree.findall("object"):
                r = {
                    "file_name": jpeg_file,
                    "image_id": fileid,
                    "height": int(tree.findall("./size/height")[0].text), # type: ignore
                    "width": int(tree.findall("./size/width")[0].text), # type: ignore
                }
                cls_ = obj.find("name").text # type: ignore
                if cls != cls_:
                    continue
                bbox = obj.find("bndbox")
                bbox = [
                    float(bbox.find(x).text) # type: ignore
    ...
    for fileid in fileids:
        ...
        for obj in tree.findall("object"):
            ...
            if not (cls in class_names):
                continue
            bbox = obj.find("bndbox")
            bbox = [float(bbox.find(x).text) for x in ["xmin", "ymin", "xmax", "ymax"]] # type: ignore
            bbox[0] -= 1.0
            bbox[1] -= 1.0
            instances.append(
                {"category_id": class_names.index(cls), "bbox": bbox, "bbox_mode": BoxMode.XYXY_ABS} # type: ignore
            )
        r["annotations"] = instances
        dicts.append(r)
    return dicts
