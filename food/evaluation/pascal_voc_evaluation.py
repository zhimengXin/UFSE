...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        for predictions_per_rank in all_predictions:
            for clsid, lines in predictions_per_rank.items(): # type: ignore
        ...
        results_2d['mAP50'] = mAP[50]

        # mAP = {iou: np.mean(x) for iou, x in aps.items()}
        mAP = [np.mean(x) for iou, x in aps.items() if iou >= 50]
        # results_2d['mAP'] = mAP[50]
        results_2d['mAP'] = np.mean(mAP)
        ...
        # Known
        results_2d.update({
            "AP50@K": np.mean(aps[50][:self.num_known_classes]),
            "P50@K": np.mean(precs[50][:self.num_known_classes]),
            "R50@K": np.mean(recs[50][:self.num_known_classes]),

            "AP75@K": np.mean(aps[75][:self.num_known_classes]),
            "P75@K": np.mean(precs[75][:self.num_known_classes]),
            "R75@K": np.mean(recs[75][:self.num_known_classes]),
        })

        if exist_base:
            results_2d.update({
                "AP50@B": np.mean(aps[50][:self.num_base_classes]),
                "P50@B": np.mean(precs[50][:self.num_base_classes]),
                "R50@B": np.mean(recs[50][:self.num_base_classes])
            })
        if exist_novel:
            results_2d.update({
                "AP50@N": np.mean(aps[50][self.num_base_classes:self.num_known_classes]),
                "P50@N": np.mean(precs[50][self.num_base_classes:self.num_known_classes]),
                "R50@N": np.mean(recs[50][self.num_base_classes:self.num_known_classes]),

                "AP75@N": np.mean(aps[75][self.num_base_classes:self.num_known_classes]),
                "P75@N": np.mean(precs[75][self.num_base_classes:self.num_known_classes]),
                "R75@N": np.mean(recs[75][self.num_base_classes:self.num_known_classes])
            })

        # Unknown
        results_2d.update({
            "AP50@U": np.mean(aps[50][-1]),
            "P50@U": np.mean(precs[50][-1]),
            "R50@U": np.mean(recs[50][-1]),

            "AP75@U": np.mean(aps[75][-1]),
            "P75@U": np.mean(precs[75][-1]),
            "R75@U": np.mean(recs[75][-1]),
        })
        apu= []
        pu = []
        ru = []
        ...
        for i in range(50, 100, 5):
            apu.append(np.mean(aps[i][-1]))
            pu.append(np.mean(precs[i][-1]))
            ru.append(np.mean(recs[i][-1]))
...
def parse_rec(filename, known_classes):
    ...
    for obj in tree.findall("object"):
        obj_struct = {}
        cls_name = obj.find("name").text # type: ignore
        ...
        obj_struct["difficult"] = int(obj.find("difficult").text) # type: ignore
...
def voc_eval(detpath, annopath, imagesetfile, classname, ovthresh=0.5, use_07_metric=False, known_classes=None):
    ...
    for imagename in imagenames:
        recs[imagename] = parse_rec(
            annopath.format(imagename), tuple(known_classes)) # type: ignore
